<script>
  ...
  $: if (tooltipData) {
    tooltipPath =
      worldAreas.find(
        (country) => country.properties.WB_A3 == tooltipData.country_code
      ) || tooltipData;
  ...
  const calculateFillColor = (lon, lat, center) => {
    ...
    return gdistance > 1.5 ? "green" : "red";
  };
</script>
      ...
      {#each taxedCircle as place}
        {#if place.lat && place.lon}
          <circle
            ...
            r="3"
            fill={calculateFillColor(place.lon, place.lat, rotatingGlobeCenter)}
            on:click={() => {
              tooltipData = place;
            }}
            on:focus={() => {
